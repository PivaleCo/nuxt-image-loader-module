...
This module allows you to place images in an arbitrary directory in your application which contains your source images. Source images are those which are most likely the largest and unaltered versions you have available. This module works by intercepting incoming image requests (on the server side) and will respond with either the source image or a _processed_ image if a query string is appended to the image URL.

Processing of images are done by defining 'image styles' in `nuxt.config.json`. You can add as many of these as suits your needs.

Images are processed using the [GraphicsMagick for node](https://github.com/aheckmann/gm) package, so please refer the documentation there to understand the variety of image processing options.
...
* `imageStyles` - This is an object containing the image processing settings where the key is the name of the image style you want to use in the image URL (see below). The value of each image style definition can contain at least one of the following key/values:
  ...
  * `macros` - This module currently only has one macro defined called `scaleAndCrop`. This does some heavy lifting to chain multiple actions together into a easy-to-read intention. We'll add to this list of macros as we find further uses for common sets of actions. If you'd like to suggest others, please open a issue, or, even better, submit a pull request.
...
3. If you make changes to your image style settings in `nuxt.config.js` the cached processed images will still exist and so you won't immediately see any change. To fix this, simply remove the cached images in the `/static/image-styles` directory or simply remove this directory altogether.
